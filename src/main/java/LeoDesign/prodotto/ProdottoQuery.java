...
public class ProdottoQuery extends TableQuery {
    ...
    String insertProdotto() {
        return String.format("INSERT INTO %s (nome, descrizione, prezzo, peso, immagine1, immagine2, immagine3) VALUES(?,?,?,?,?,?,?);", table);
    }
    String updateProdotto() {
        return String.format("UPDATE FROM %s SET nome=?, descrizione=?, prezzo=?, peso=?, immagine1=?, immagine2=?, immagine3=? WHERE idProdotto=?;", table);
    }
